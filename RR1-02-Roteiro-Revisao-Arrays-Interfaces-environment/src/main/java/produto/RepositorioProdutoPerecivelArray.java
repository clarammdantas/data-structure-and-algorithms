 ...
 *
 */
public class RepositorioProdutoPerecivelArray implements QualquerRepositorio {
	...
	public boolean existe(int codigo) {
		int contemProduto = procurarIndice(codigo);
		return contemProduto != -1;
	}
	...
	public void remover(int codigo) {
		...
		if (produtoIndex != -1) {
			produtos[produtoIndex] = produtos[index--];
			remover(codigo);
		}
	}
	...
	public ProdutoPerecivel procurar(int codigo) {
		...
		if (produtoIndex == -1)
			...
			p = (ProdutoPerecivel) produtos[produtoIndex];
		
		return (ProdutoPerecivel) p;
	}
}
